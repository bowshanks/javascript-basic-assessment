...
var secondHero = heroes.slice(heroes.indexOf("flash"),1);
// leave batman in the array but put a copy of him on a variable called thirdHero
var thirdHero = heroes.find(function(currentValue){if (currentValue ==="batman"){return currentValue}});
...
function addItem(arr,item){
	arr.indexOf(item) > -1 ? arr : arr.push(item);
	return arr;
}
...
function doubleTheFun (toDouble) {
	return isNaN(toDouble) ? toDouble + toDouble : Number(toDouble) + Number(toDouble);
}
